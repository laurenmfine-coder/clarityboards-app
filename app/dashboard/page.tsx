...
function DetailModal({ item, onUpdate, onDelete, onClose, boardNames = {} }: {
  ...
  const uploadAttachment = async (checklistItemId: string, file: File) => {
    if (file.size > 10485760) { alert('File must be under 10 MB'); return }
    const existing = item.checklist.find(c => c.id === checklistItemId)?.attachments ?? []
    if (existing.length >= 5) { alert('Maximum 5 attachments per checklist item'); return }
    setUploadingFor(checklistItemId)
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) return
      const ext = file.name.split('.').pop()
      const storagePath = session.user.id + '/' + item.id + '/' + checklistItemId + '_' + Date.now() + '.' + ext
      const { error } = await supabase.storage.from('checklist-attachments').upload(storagePath, file, { upsert: false })
      if (error) { alert('Upload failed: ' + error.message); return }
      const { data: urlData } = supabase.storage.from('checklist-attachments').getPublicUrl(storagePath)
      const attachment = { name: file.name, url: urlData.publicUrl, type: file.type }
      onUpdate({ checklist: item.checklist.map(c => c.id === checklistItemId ? { ...c, attachments: [...(c.attachments ?? []), attachment] } : c) })
    } finally { setUploadingFor(null) }
  }
  const deleteAttachment = async (checklistItemId: string, url: string, name: string) => {
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) return
    const pathMatch = url.match(/checklist-attachments\/(.+)$/)
    if (pathMatch) await supabase.storage.from('checklist-attachments').remove([pathMatch[1]])
    onUpdate({ checklist: item.checklist.map(c => c.id === checklistItemId ? { ...c, attachments: (c.attachments ?? []).filter(a => a.url !== url) } : c) })
  }
...
function WeekView({ items, weekStart, setWeekStart, onItemClick, onDayClick }: { items: Item[]; weekStart: Date; setWeekStart: (d: Date) => void; onItemClick: (item: Item) => void; onDayClick?: (dateStr: string, e: React.MouseEvent) => void }) {
  ...
  const weekLabel = s.getMonth() === e.getMonth() ? mo + ' ' + s.getDate() + '-' + e.getDate() + ', ' + s.getFullYear() : mo + ' ' + s.getDate() + ' - ' + mo2 + ' ' + e.getDate() + ', ' + e.getFullYear()
  const prevWeek = () => { const d = new Date(weekStart); d.setDate(d.getDate()-7); setWeekStart(d) }
  const nextWeek = () => { const d = new Date(weekStart); d.setDate(d.getDate()+7); setWeekStart(d) }
  const goToday = () => { const d = new Date(); d.setHours(0,0,0,0); const day = d.getDay(); const diff = day === 0 ? -6 : 1-day; d.setDate(d.getDate()+diff); setWeekStart(d) }
  const unscheduled = items.filter(i => !i.date)
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ fontFamily: T.serif, fontSize: 22, color: T.ink, fontWeight: 500 }}>{weekLabel}</div>
          <button onClick={goToday} style={{ fontSize: 11, padding: '3px 10px', borderRadius: 20, border: '1px solid ' + T.border, background: T.ivory, color: T.sub, cursor: 'pointer', fontFamily: T.sans }}>Today</button>
        </div>
        <div style={{ display: 'flex', gap: 4 }}>
          <button onClick={prevWeek} style={{ width: 32, height: 32, borderRadius: '50%', border: '1px solid ' + T.border, background: T.ivory, color: T.sub, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{'<'}</button>
          <button onClick={nextWeek} style={{ width: 32, height: 32, borderRadius: '50%', border: '1px solid ' + T.border, background: T.ivory, color: T.sub, cursor: 'pointer', fontSize: 16, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{'>'}</button>
        </div>
      </div>
      <style>{'.cb-week-grid{display:grid;grid-template-columns:repeat(7,1fr);gap:4px}@media(max-width:600px){.cb-week-grid{grid-template-columns:repeat(3,1fr)}}'}</style>
            ...
            <div key={dateStr} onClick={e => { if (onDayClick) onDayClick(dateStr, e) }}
              ...
              <div style={{ marginBottom: 6, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <div>
                  ...
                  <div style={{ display: 'inline-flex', width: 22, height: 22, alignItems: 'center', justifyContent: 'center', borderRadius: '50%', fontSize: 12, fontWeight: 600, background: isToday ? T.ink : 'transparent', color: isToday ? '#fff' : T.ink }}>{day.getDate()}</div>
                </div>
                ...
                {dayItems.map(item => {
                  ...
                  return (
                    <button key={item.id} onClick={e => { e.stopPropagation(); onItemClick(item) }}
                      style={{ width: '100%', textAlign: 'left', padding: '3px 5px', borderRadius: 4, border: 'none', cursor: 'pointer', fontSize: 11, fontWeight: 500, color: cfg?.color, background: (cfg?.color ?? '') + '14', borderLeft: '2px solid ' + (cfg?.color ?? ''), overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', fontFamily: T.sans }}>
...
export default function Dashboard() {
  ...
  const loadActivity = useCallback(async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session) return
      const { data } = await supabase.from('notification_log').select('*').order('created_at', { ascending: false }).limit(100)
  ...
  const loadUserTemplates = useCallback(async () => {
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) return
    const { data } = await supabase.from('user_templates').select('*').order('created_at', { ascending: false })
    setUserTemplates(data ?? [])
  }, [])

  const saveUserTemplate = async (name: string, board: string, visibleItems: Item[]) => {
    const { data: { session } } = await supabase.auth.getSession()
    if (!session) return
    const templateItems = visibleItems.map(i => ({ title: i.title }))
    const { data } = await supabase.from('user_templates').insert({ name, board, items: templateItems, user_id: session.user.id }).select().single()
    if (data) setUserTemplates(prev => [data, ...prev])
  }
  ...
  return (
    <div style={{ minHeight: '100vh', background: T.cream, fontFamily: T.sans }}>
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '0 20px' }}>
          <div style={{ display: 'flex', alignItems: 'center', height: 54, gap: 14 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
              ...
              <NavIconBtn onClick={() => { setShowActivity(s => !s); if (!showActivity) loadActivity() }} title="Activity Feed">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 28, height: 50, overflowX: 'auto', paddingRight: 4 }}>
            ...
            <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 10, flexShrink: 0 }}>
              <div style={{ display: 'flex', border: `0.5px solid ${T.border}`, borderRadius: 4, overflow: 'hidden' }}>
                {([['list', <List size={12} strokeWidth={1.5} />], ['week', <AlignJustify size={12} strokeWidth={1.5} />], ['calendar', <LayoutGrid size={12} strokeWidth={1.5} />]] as const).map(([v, icon]) => (
          ...
          return (
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 28, paddingBottom: 20, borderBottom: `0.5px solid ${T.border}` }}>
              ...
              <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
                ...
                <SaveTemplateButton board={activeBoard} items={filtered} onSave={saveUserTemplate} />
              </div>
            </div>
          )
        })()}
      ...
      {showTemplates && <TemplatesModal onApply={items => { items.forEach(p => addItem(p)); setShowTemplates(false) }} onClose={() => setShowTemplates(false)} userTemplates={userTemplates} onDeleteUserTemplate={deleteUserTemplate} />}
      {showInbox && (
        <>
        ...
        <>
          ...
          <div style={{ position: 'fixed', top: 52, right: 12, width: 380, maxHeight: 'calc(100dvh - 70px)', background: T.ivory, borderRadius: 12, boxShadow: '0 4px 32px rgba(26,23,20,0.18)', border: '1px solid ' + T.border, zIndex: 45, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            ...
            <div style={{ overflowY: 'auto', flex: 1 }}>
              {activityLoading ? (
                <div style={{ padding: 24, textAlign: 'center', color: T.muted, fontSize: 13, fontFamily: T.sans }}>Loading...</div>
              ) : activityFeed.filter(n => activityBoardFilter === 'all' || n.board === activityBoardFilter).length === 0 ? (
                <div style={{ padding: 32, textAlign: 'center' }}>
                  <div style={{ fontSize: 13, color: T.muted, fontFamily: T.sans }}>No activity yet</div>
                </div>
              ) : (
                activityFeed.filter(n => activityBoardFilter === 'all' || n.board === activityBoardFilter).map(n => {
                  const cfg = BOARD_MAP[n.board as keyof typeof BOARD_MAP]
                  const diff = Date.now() - new Date(n.created_at).getTime()
                  const mins = Math.floor(diff / 60000)
                  ...
                  return (
                    <div key={n.id} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, padding: '12px 16px', borderBottom: '1px solid ' + T.border }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontSize: 13, color: T.ink, fontFamily: T.sans, lineHeight: 1.4 }}>
                          {n.actor_email && <span style={{ fontWeight: 600 }}>{n.actor_email.split('@')[0]} </span>}
                          <span style={{ color: T.sub }}>{actionVerb[n.action] ?? n.action} </span>
                          <span style={{ fontWeight: 500 }}>'{n.item_title}'</span>
                          {n.detail && <span style={{ color: T.muted }}> - {n.detail}</span>}
                        </div>
                        <div style={{ fontSize: 11, color: T.muted, fontFamily: T.sans, marginTop: 3 }}>{cfg?.label ?? n.board} - {timeAgo}</div>
                      </div>
                    </div>
                  )
                })
...
// ─── Board Templates ──────────────────────────────────────
function SaveTemplateButton({ board, items, onSave }: { board: string; items: Item[]; onSave: (name: string, board: string, items: Item[]) => Promise<void> }) {
  const [saving, setSaving] = useState(false)
  const [showInput, setShowInput] = useState(false)
  const [name, setName] = useState('')
  if (board === 'all') return null
  const handle = async () => {
    if (!name.trim()) return
    setSaving(true)
    await onSave(name.trim(), board, items)
    setSaving(false); setShowInput(false); setName('')
  }
  if (showInput) return (
    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
      <input autoFocus value={name} onChange={e => setName(e.target.value)}
        onKeyDown={e => { if (e.key === 'Enter') handle(); if (e.key === 'Escape') { setShowInput(false); setName('') } }}
        placeholder='Template name...'
        style={{ fontSize: 12, padding: '6px 10px', borderRadius: 4, border: '0.5px solid ' + T.border, fontFamily: T.sans, color: T.ink, background: T.ivory, width: 150, outline: 'none' }} />
      <button onClick={handle} disabled={saving || !name.trim()}
        style={{ padding: '6px 12px', borderRadius: 4, border: 'none', background: T.ink, color: 'white', fontSize: 12, fontWeight: 500, cursor: 'pointer', fontFamily: T.sans, opacity: saving || !name.trim() ? 0.5 : 1 }}>
        {saving ? '...' : 'Save'}
      </button>
      <button onClick={() => { setShowInput(false); setName('') }}
        style={{ padding: '6px 8px', borderRadius: 4, border: '0.5px solid ' + T.border, background: 'white', color: T.sub, fontSize: 12, cursor: 'pointer', fontFamily: T.sans }}>X</button>
    </div>
  )
  return (
    <button onClick={() => setShowInput(true)}
      style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 13px', borderRadius: 4, border: '0.5px solid ' + T.border, color: T.inkMid, fontSize: 12, fontWeight: 500, background: 'white', cursor: 'pointer', fontFamily: T.sans }}>
      <Sparkles size={12} strokeWidth={1.5} /> Save as template
    </button>
  )
}
...
function TemplatesModal({ onApply, onClose, userTemplates = [], onDeleteUserTemplate = (_id: string) => {} }: { onApply: (items: Partial<Item>[]) => void; onClose: () => void; userTemplates?: any[]; onDeleteUserTemplate?: (id: string) => void }) {
